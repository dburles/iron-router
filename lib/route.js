...
Route.prototype = {
  ...
  params: function (path) {
    ...
    _.extend(params, this.queryParams(path));
  ...
  queryParams: function (path) {
    ...
    if (queryString) {
      _.each(queryString.split('&'), function (paramString) {
        ...
        // Convert same named fields into an array. e.g.
        // foo[]=1&foo[]=2&foo[]=3 returns { foo: [1, 2, 3] }
        if (key.indexOf('[]') !== -1) {
    ...
    if (path instanceof RegExp) {
      ...
      if (_.isObject(query)) {
        query = _.map(_.pairs(query), function (queryPart) {
          if (_.isArray(queryPart[1])) {
            return _.map(queryPart[1], function(part) {
              return queryPart[0] + '=' + encodeURIComponent(part);
            }).join('&');
          } else
  ...
  newController: function (path, options) {
    ...
    options = _.extend({}, options, {
      ...
      queryParams: this.queryParams(path),
      where: this.where,
      action: this.action
    });
